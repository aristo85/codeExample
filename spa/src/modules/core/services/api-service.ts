...
export type ApiServiceInterface = {
  ...
  updatePhoto: <Response = unknown, Data = unknown>(
    url: string,
    data?: Data
  ) => Promise<Response>;
};
...
class ApiService implements ApiServiceInterface {
  ...
  public async updatePhoto<Response = unknown, Data = unknown>(
    url,
    data: Data | any
  ): Promise<Response> {
    const bearer = `Bearer ${this.token}`;
    ...
    return (
      axios
        .put(`${config.apiUrl}${url}`, fileData, conf)
        // .catch(this.handleError)
        .then(this.checkStatus)
        // .then(this.parseJSON)
        .catch((error) => {
  ...
  public async createKycAccessToken() {
    return this.createResource('/api/v1/access_tokens');
  }
}
