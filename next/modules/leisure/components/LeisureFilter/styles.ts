...
export const LeisureFilterWrapper = styled.div`
  position: relative;
  padding: 16px;
  gap: 24px;
  max-width: 1180px;
...
export const LeisureFilterForm = styled.form`
  display: flex;
  flex-wrap: wrap;
  gap: 24px;
  width: 100%;
...
export const LeisureFilterSelect = styled(InputSelect)`
  & .optionsMenu {
    max-height: 400px;
    overflow-y: scroll;
    overflow-x: hidden;
...
export const LeisureFilterDateInput = styled(DateInput)`
  input {
    padding: 3px 0 0 16px;
  }
`;

export const LeisureFilterButton = styled(ButtonBasic)`
  width: 100%;
  height: 63px;
  align-self: center;
`;
