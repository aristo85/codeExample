...
export const ExcursionFilterWrapper = styled.div`
  position: relative;
  padding: 16px;
  gap: 24px;
  max-width: 1180px;
...
export const ExcursionFilterForm = styled.form`
  display: flex;
  flex-wrap: wrap;
  gap: 24px;
  width: 100%;
...
export const ExcursionFilterSelect = styled(InputSelect)`
  & .optionsMenu {
    max-height: 400px;
    overflow-y: scroll;
    overflow-x: hidden;
...
export const ExcursionFilterDateInput = styled(DateInput)`
  input {
    padding: 3px 0 0 16px;
  }
`;

export const ExcursionFilterButton = styled(ButtonBasic)`
  width: 100%;
  height: 63px;
  align-self: center;
`;
